...
def extract_peak(heatmap, max_pool_ks=7, min_score=-5, max_det=100):
    ...
    W = heatmap.shape[1]
    max_element = H*W
    heatmap4d = heatmap[None, None]
    max_pool_out = F.max_pool2d(heatmap4d, kernel_size=max_pool_ks, stride=1, padding=max_pool_ks // 2)

    # get the peaks
    peaks = torch.where(torch.eq(heatmap4d.view(-1), max_pool_out.view(-1)), heatmap4d.view(-1), min_val-1)

    # filter values less than min_score
    peaks = torch.where(torch.gt(peaks, min_score), peaks, min_val-1)

    # capping the max_det
    if max_det > max_element:
        max_det = max_element

    values = torch.topk(peaks, max_det).values
    indices = torch.topk(peaks, max_det).indices
    for val, index in zip(values.tolist(), indices.tolist()):
        if val == min_val - 1:
            ...
            cx = int(index % W)
            cy = int(index // W)
            extracted_peaks.append((val, cx, cy))
...
class Detector(torch.nn.Module):
    ...
    def __init__(self, layers=[16, 32, 64, 128], n_output_channels=3, kernel_size=3, use_skip=True):
        ...
        self.classifier = torch.nn.Sequential(
            torch.nn.Conv2d(c, n_output_channels, 1),
        )
    ...
    def detect(self, image):
        heatmap3c = self.forward(image)
        if len(heatmap3c.size()) == 4:
            heatmap3c = torch.squeeze(heatmap3c)

        detections_all_channels = {}
        for channel in range(3):
            heatmap = heatmap3c[channel, :, :]
            extracted_peaks = extract_peak(heatmap, max_det=30)
            detections_per_channel = []
            for detection in extracted_peaks:
                detection = detection+(0.0, 0.0)
                detections_per_channel.append(detection)
            detections_all_channels[str(channel)] = detections_per_channel
        return detections_all_channels['0'], detections_all_channels['1'], detections_all_channels['2']
